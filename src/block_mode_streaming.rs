      ...
      R: Read,
{
    ...
    fn fill_buf(&mut self) -> Result<usize> {
        ...
        if read_size % Self::BLOCK_SIZE != 0 {
            return Err(Error::new(ErrorKind::InvalidInput, "number of bytes in reader was not a multiple of block size"));
        }
    ...
    fn read(&mut self, buf: &mut [u8]) -> Result<usize> {
        ...
        loop {
            ...
            if buf.len() - read_size >= local_read_size {
                buf[read_size..read_size + local_read_size].copy_from_slice(&self.buf[self.filled_buf.clone()]);
      ...
      W: Write,
{
    ...
    pub fn new(block_mode: T, writer: W, direction: Direction) -> Self {
        Self { block_mode, writer, direction, buf: vec![0u8; Self::BLOCK_SIZE * 8], buf_bytes: 0, _c: PhantomData, _p: PhantomData }
    }
}
      ...
      W: Write,
{
    fn write(&mut self, buf: &[u8]) -> Result<usize> {
        ...
        while bytes_written < buf.len() {
            ...
            let bytes_to_copy_into_local_buf = min(self.buf.len() - self.buf_bytes, buf.len() - bytes_written);
            ...
            self.writer.write_all(&self.buf[..bytes_to_write_immediately])?;
            // Move any remaining bytes to the beginning of the buffer
            self.buf.copy_within(bytes_to_write_immediately..self.buf_bytes, 0);
    ...
    fn flush(&mut self) -> Result<()> {
        if self.buf_bytes != 0 {
            Err(Error::new(ErrorKind::InvalidData, "the number of bytes written was not a multiple of block size"))
        } else {
            self.writer.flush()
        }
    }
